...
function form_start($action, $method="post", $enc=FALSE) {
  echo '<table class="myformtable">', "\n",
       '<form  ', html_attribute("method", $method),
                  html_attribute("action", $action);
  if($enc)
    echo html_attribute("enctype", $enc);
  echo ">\n"; }
...
function form_label($caption, $necessary=FALSE) {
  ...
  if($necessary)
    echo '<span class="red">', nl2br(htmlentities($caption)), '</span>';
  else
    echo htmlentities($caption);
  echo '</td>', "\n";
}
...
function form_caption($caption, $colspan=1) {
  if($colspan>1)
    ...
    echo '<td class="myformtd">';
  echo nl2br(htmlentities($caption)), '</td>', "\n";
}
...
function form_url($url, $txt, $colspan=1) {
  if($colspan>1)
    ...
    echo '<td class="myformtd">';
  echo "<a href=\"$url\">" . htmlentities($txt) . "</a></td>\n";
}
...
// create select list
function form_list($name, $rows, $selected=-1, $default=FALSE) {
  if(!$default)
    $default="(choose)";
  echo '<td class="myformtd">';
  echo '<select class="mycontrol" ',
    html_attribute("name", "form[$name]"), '>', "\n";
  echo '<option value="none">', htmlentities($default), '</option>', "\n";
  foreach($rows as $row) {
...
function form_file($name, $size=40) {
  echo '<td class="myformtd"><input ',
    html_attribute("class", "mybutton"),
    html_attribute("type", "file"),
    html_attribute("size", $size),
